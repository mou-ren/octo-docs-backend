...
export const config = {
  ...
  // READS it — candidates come back from OS in relevance order, then the DB
  // visibility model (docMetaRepo.filterVisibleDocIds) filters to what the caller
  // may see. No permissions live in OS. Disabled by default (gray release): with
  // SEARCH_ENABLED=false the POST /docs/search route returns 503 and never
